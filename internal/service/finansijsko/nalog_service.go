...
type NalogService interface {
	...
	GetNalogStampaData(ctx context.Context, idFnal int64, cfg config.Config) (domain.Fnal, domain.TableData, domain.TableData, error)
	GetGrupeNaloziStampaData(ctx context.Context, tipdok string, odBroja, doBroja int64, cfg config.Config) ([]domain.NalogStampaItem, domain.TableData, error)
...
func (s *NalogResource) GetFvrData(ctx context.Context) (domain.Fvr, error) {
	if s.fvrRepo == nil {
...
func (s *NalogResource) GetAll(ctx context.Context, page int, offset int, tableFields []domain.Fields, idField string, searchText, sortBy, sortOrder string) (*[]domain.Fnal, error) {
...
func (s *NalogResource) GetTotalRecords(ctx context.Context, tableFields []domain.Fields, searchText string) (int, error) {
...
func (s *NalogResource) GetNalogViewData(ctx context.Context, tbl *domain.TableData, searchQuery string, currentPage, pageSize int, isInitialRequest bool, sortBy, sortOrder, tipdok string) (NalogViewData, error) {
	...
	if searchQuery != "" {
		nbrArgs := len(qbCount.GetArgs())
		qbCount.AddCustomCondition(fmt.Sprintf("(tipdok ILIKE $%d OR nalog::TEXT ILIKE $%d OR opis ILIKE $%d OR dug::TEXT ILIKE $%d OR pot::TEXT ILIKE $%d OR datob::TEXT ILIKE $%d OR brst::TEXT ILIKE $%d OR nalsts ILIKE $%d)", nbrArgs+1, nbrArgs+1, nbrArgs+1, nbrArgs+1, nbrArgs+1, nbrArgs+1, nbrArgs+1, nbrArgs+1), searchQuery)
	}
	...
	qb.AddEqual("tipdok", tipdok)
	if searchQuery != "" {
		nbrArgs := len(qb.GetArgs())
		qb.AddCustomCondition(fmt.Sprintf("(tipdok ILIKE $%d OR nalog::TEXT ILIKE $%d OR opis ILIKE $%d OR dug::TEXT ILIKE $%d OR pot::TEXT ILIKE $%d OR datob::TEXT ILIKE $%d OR brst::TEXT ILIKE $%d OR nalsts ILIKE $%d)", nbrArgs+1, nbrArgs+1, nbrArgs+1, nbrArgs+1, nbrArgs+1, nbrArgs+1, nbrArgs+1, nbrArgs+1), searchQuery)
	}
	if sortBy != "" {
	...
	if isInitialRequest {
		ukObrada, err := s.GetNaloziTotals(ctx, tipdok)
		if err != nil {
			return viewData, fmt.Errorf("failed to get Nalozi totals: %w", err)
		}
...
func (s *NalogResource) GetNalogPrepisData(ctx context.Context, tbl *domain.TableData, currentPage, pageSize int, searchText, sortBy, sortOrder string) error {
	...
	if searchText != "" {
		...
		argsNbr := len(qbCount.GetArgs())
		qbCount.AddCustomCondition(fmt.Sprintf("(tipdok ILIKE $%d OR nalog::TEXT ILIKE $%d OR opis ILIKE $%d OR dug::TEXT ILIKE $%d OR pot::TEXT ILIKE $%d OR datob::TEXT ILIKE $%d OR brst::TEXT ILIKE $%d OR nalsts ILIKE $%d)", argsNbr+1, argsNbr+1, argsNbr+1, argsNbr+1, argsNbr+1, argsNbr+1, argsNbr+1, argsNbr+1), likePattern)
	}
	sqlQuery, args := qbCount.Build()
	totRecords, err := s.fnalRepo.GetTotalRecordsCustom(ctx, sqlQuery, "", args, "", "")
	if err != nil {
	...
	if searchText != "" {
		likePattern := fmt.Sprintf("%%%s%%", searchText)
		argsNbr := len(qb.GetArgs())
		qb.AddCustomCondition(fmt.Sprintf("(tipdok ILIKE $%d OR nalog::TEXT ILIKE $%d OR opis ILIKE $%d OR dug::TEXT ILIKE $%d OR pot::TEXT ILIKE $%d OR datob::TEXT ILIKE $%d OR brst::TEXT ILIKE $%d OR nalsts ILIKE $%d)", argsNbr+1, argsNbr+1, argsNbr+1, argsNbr+1, argsNbr+1, argsNbr+1, argsNbr+1, argsNbr+1), likePattern)
	}
	if sortBy != "" {
		qb.AddOrderBy(fmt.Sprintf("fnal.%s", sortBy))
	}
...
func (s *NalogResource) GetNalogStornirajData(ctx context.Context, tbl *domain.TableData, currentPage, pageSize int, searchText, sortBy, sortOrder string) (NalogViewData, error) {
	...
	if searchText != "" {
		nbrArgs := len(qbCount.GetArgs())
		qbCount.AddCustomCondition(fmt.Sprintf("(tipdok::TEXT ILIKE '%%' || $%d || '%%' OR nalog::TEXT ILIKE '%%' || $%d || '%%' OR opis ILIKE '%%' || $%d || '%%' OR dug::TEXT ILIKE '%%' || $%d || '%%' OR pot::TEXT ILIKE '%%' || $%d || '%%' OR datob::TEXT ILIKE '%%' || $%d || '%%' OR brst::TEXT ILIKE '%%' || $%d || '%%' OR nalsts ILIKE '%%' || $%d || '%%')", nbrArgs+1, nbrArgs+1, nbrArgs+1, nbrArgs+1, nbrArgs+1, nbrArgs+1, nbrArgs+1, nbrArgs+1), searchText)
	}
	...
	if searchText != "" {
		nbrArgs := len(qb.GetArgs())
		qb.AddCustomCondition(fmt.Sprintf("(tipdok::text ilike '%%' || $%d || '%%' OR nalog::text ilike '%%' || $%d || '%%' OR opis ilike '%%' || $%d || '%%' OR datob::text ilike '%%' || $%d || '%%' OR dug::text ilike '%%' || $%d || '%%' OR pot::text ilike '%%' || $%d || '%%' OR nalsts::text ilike '%%' || $%d || '%%')", nbrArgs+1, nbrArgs+1, nbrArgs+1, nbrArgs+1, nbrArgs+1, nbrArgs+1, nbrArgs+1), searchText)
	}
	if sortBy != "" {
		qb.AddOrderBy(fmt.Sprintf("fnal.%s", sortBy))
	}
...
func (s *NalogResource) GetNalogStampanjeData(ctx context.Context, tbl *domain.TableData, getTotalRecords bool, page, pageSize int, searchText, sortBy, sortOrder string) error {
	...
	if searchText != "" {
		args := qb.GetArgs()
		qb.AddCustomCondition(fmt.Sprintf("(tipdok::text ilike '%%' || $%d || '%%' OR nalog::text ilike '%%' || $%d || '%%' OR opis ilike '%%' || $%d || '%%' OR datob::text ilike '%%' || $%d || '%%' OR dug::text ilike '%%' || $%d || '%%' OR pot::text ilike '%%' || $%d || '%%' OR nalsts::text ilike '%%' || $%d || '%%')", len(args)+1, len(args)+1, len(args)+1, len(args)+1, len(args)+1, len(args)+1, len(args)+1), searchText)
	}
	if !getTotalRecords {
		if sortBy != "" {
			qb.AddOrderBy(fmt.Sprintf("fnal.%s", sortBy))
...
func (s *NalogResource) GetNalogStampanjeDataDetalji(ctx context.Context, tbl *domain.TableData, getTotalRecords bool, page, pageSize int, idFnal int64, searchText, sortBy, sortOrder string) error {
	...
	if searchText != "" {
		nbrArgs := len(qb.GetArgs())
		qb.AddCustomCondition(fmt.Sprintf("(fpro.rbr::text ilike '%%' || $%d || '%%' OR fpro.konto::text ilike '%%' || $%d || '%%' OR fpro.sifra::text ilike '%%' || $%d || '%%' OR fkpl.naziv::text ilike '%%' || $%d || '%%' OR fpro.vrd::text ilike '%%' || $%d || '%%' OR fpro.opis::text ilike '%%' || $%d || '%%' OR fpro.iznos::text ilike '%%' || $%d || '%%' OR fpro.dokum::text ilike '%%' || $%d || '%%' OR fpro.dadok::text ilike '%%' || $%d || '%%' OR fpro.vkonta::text ilike '%%' || $%d || '%%')", nbrArgs+1, nbrArgs+1, nbrArgs+1, nbrArgs+1, nbrArgs+1, nbrArgs+1, nbrArgs+1, nbrArgs+1, nbrArgs+1, nbrArgs+1), searchText)
	}
	if !getTotalRecords {
		if sortBy != "" {
...
func (s *NalogResource) GetNaloziTotals(ctx context.Context, tipdok string) (domain.UkupnaObrada, error) {
	userSession := domain.GetSessionFromStdContext(ctx)
	if userSession == nil {
...
func (s *NalogResource) GetNalogTotalValues(ctx context.Context, nalogTotals *domain.NalogTotalValues, idFnal int64) error {
...
func (s *NalogResource) GetNalogStampaData(ctx context.Context, idFnal int64, cfg config.Config) (domain.Fnal, domain.TableData, domain.TableData, error) {
	userSession := domain.GetSessionFromStdContext(ctx)
	if userSession == nil {
	...
	qb := common.NewQueryBuilder(`SELECT fpro.rbr, fpro.konto, coalesce(fpro.sifra, '') as sifra,
		...
		FROM fpro `, true)
	qb.AddJoin(` left join fkpl ON fpro.idfkpl = fkpl.idfkpl`)
	qb.AddJoin(` left join lateral (SELECT naziv FROM partneri WHERE sifra = fpro.sifra AND tipanalitikeid = fkpl.tipanalitikeid LIMIT 1) p ON true`)
	...
	if fproEntities != nil {
		for _, entity := range *fproEntities {
			...
			if _, exists := kontoMap[key]; !exists {
				...
				kontoMap[key] = &kontoAgg{}
			}

			kontoMap[key].BrStavki++
			kontoMap[key].Duguje += dug
			kontoMap[key].Pot += pot
		}
	}
...
// GetGrupeNaloziStampaData fetches data for all nalozi matching the given criteria for group printing.
// Returns a slice of NalogStampaItem (one per nalog) and a grand konta summary across all nalozi.
func (s *NalogResource) GetGrupeNaloziStampaData(ctx context.Context, tipdok string, odBroja, doBroja int64, cfg config.Config) ([]domain.NalogStampaItem, domain.TableData, error) {
	userSession := domain.GetSessionFromStdContext(ctx)
	if userSession == nil {
		return nil, domain.TableData{}, fmt.Errorf("user session not found")
	}
	...
	grandKontoOrder := []string{}
	grandKontoMap := map[string]*kontoAgg{}
	var items []domain.NalogStampaItem

	qb := common.NewQueryBuilder(`SELECT
			fnal.nalog, fnal.tipdok, fnal.danal, fnal.opis,
			fpro.rbr, fpro.konto, coalesce(fpro.sifra, '') as sifra,
			case when fpro.sifra = '' then coalesce(fkpl.naziv, '') else coalesce(p.naziv, '') end as naziv,
			fpro.vrd,
			coalesce(o.ojozn, '') as ojozn,
			...
			case when fpro.kat in (3,4) then fpro.iznos else 0 end as pot
			FROM fnal`, true)
	qb.AddJoin(`left join fpro ON fnal.idfnal = fpro.idfnal`)
	qb.AddJoin(`left join fkpl ON fpro.idfkpl = fkpl.idfkpl`)
	qb.AddJoin(`left join lateral (SELECT naziv FROM partneri WHERE sifra = fpro.sifra AND tipanalitikeid = fkpl.tipanalitikeid LIMIT 1) p ON true`)
	qb.AddJoin(`left join orgjed o ON o.idorgjed = fpro.idorgjed`)
	if hasGod {
		qb.AddEqual("fnal.god", userSession.SelectedGod)
	}
	if hasKar {
		qb.AddEqual("fnal.kar", userSession.SelectedKar)
	}
	qb.AddEqual("fnal.tipdok", tipdok)
	qb.AddCondition("fnal.nalog", odBroja, ">=")
	qb.AddCondition("fnal.nalog", doBroja, "<=")
	qb.AddOrderBy("fnal.nalog, fpro.rbr")
	qb.AddSortOrder("ASC")

	sqlQuery, args := qb.Build()
	entities, err := s.fproRepo.GetAllCustom(ctx, sqlQuery, "", args, "", "")
	if err != nil {
		return nil, domain.TableData{}, fmt.Errorf("failed to get stavke: %w", err)
	}
	if entities == nil || len(*entities) == 0 {
		return items, domain.TableData{}, nil
	}

	// Helper to build konta summary TableData from an ordered konto map.
	buildKontaSummary := func(order []string, kmap map[string]*kontoAgg) domain.TableData {
		tbl := domain.TableData{
			HasTotals: true,
			Headers: []domain.Fields{
				{Label: "konto"},
				{Label: "broj stavki", TextAlign: "right", IncludeInTotals: true},
				{Label: "duguje", TextAlign: "right", IncludeInTotals: true},
				{Label: "potrazuje", TextAlign: "right", IncludeInTotals: true},
				{Label: "saldo", TextAlign: "right", IncludeInTotals: true},
			},
		}
		var totBr int
		...
		tbl.Totals = []string{
			i18n.GetInstance().Label("Ukupno"),
			fmt.Sprintf("%d", totBr),
			common.FormatNumberWithSystemLocale(totDug, 2),
			common.FormatNumberWithSystemLocale(totPot, 2),
			common.FormatNumberWithSystemLocale(totDug-totPot, 2),
		}
		return tbl
	}

	// Per-nalog grouping state
	var currentNalog int64 = -1
	var currentFnal domain.Fnal
	var tblStavke domain.TableData
	var kontoOrder []string
	var kontoMap map[string]*kontoAgg
	var totalDug, totalPot float64
	...
	finalizeNalog := func() {
		...
		tblStavke.Totals = make([]string, len(tblStavke.Headers))
		for i, h := range tblStavke.Headers {
			switch h.Name {
			case "dug":
				tblStavke.Totals[i] = common.FormatNumberWithSystemLocale(totalDug, 2)
			case "pot":
		...
		items = append(items, domain.NalogStampaItem{
			Fnal:         currentFnal,
			Stavke:       tblStavke,
			KontaSummary: buildKontaSummary(kontoOrder, kontoMap),
		})
	}

	for _, entity := range *entities {
		// Detect nalog boundary
		if entity.Nalog != currentNalog {
			finalizeNalog()
			currentNalog = entity.Nalog
			currentFnal = domain.Fnal{Nalog: entity.Nalog, Tipdok: entity.Tipdok, Opis: entity.Opis}
			if entity.Danal != nil {
				currentFnal.Danal = *entity.Danal
			}
			tblStavke = common.SetTableBasicData("", fmt.Sprintf("tbl-grp-%d", entity.Nalog),
				s.nalogGrupaStampaTableFields, "", "", 999999, 1, 1, 0, cfg)
			tblStavke.HasTotals = true
			kontoOrder = []string{}
			kontoMap = map[string]*kontoAgg{}
			totalDug = 0
		...
		// Skip left-join null rows (nalog without stavke)
		if entity.Rbr == 0 {
			continue
		}
		...
		dadokStr := ""
		if entity.Dadok.Valid {
			dadokStr = entity.Dadok.Time.Format("02.01.2006")
		}
		...
		if _, exists := grandKontoMap[key]; !exists {
			grandKontoOrder = append(grandKontoOrder, key)
			grandKontoMap[key] = &kontoAgg{}
		}
		grandKontoMap[key].BrStavki++
		grandKontoMap[key].Duguje += dug
		grandKontoMap[key].Pot += pot
	}
	finalizeNalog() // flush the last nalog
...
func (s *NalogResource) setServiceFieldValues() {
	...
	s.naloziStavkeStampaTableFields = []domain.Fields{
		...
		{Name: "vrd", Label: "Vrsta Dokumenta", Width: "10", Sortable: true},
	...
	s.nalogGrupaStampaTableFields = []domain.Fields{
		{Name: "rbr", Label: "Redni Broj", Width: "4", TextAlign: "right"},
		{Name: "konto", Label: "Konto", Width: "6"},
		{Name: "sifra", Label: "Šifra", Width: "6"},
		{Name: "naziv", Label: "Naziv konta", Width: "30"},
		{Name: "vrd", Label: "Vrsta Dokumenta", Width: "5", TextAlign: "right"},
		{Name: "ojozn", Label: "ID OJ", Width: "7"},
		{Name: "dokum", Label: "Br. dokumenta", Width: "12"},
		{Name: "dadok", Label: "Datum dokumenta", Width: "10"},
		{Name: "rok", Label: "Rok", Width: "4", TextAlign: "right"},
		{Name: "vkonta", Label: "Nacin knjizenja", Width: "5", TextAlign: "right"},
		{Name: "dug", Label: "Duguje", Width: "12", TextAlign: "right", IncludeInTotals: true},
		{Name: "pot", Label: "Potražuje", Width: "12", TextAlign: "right", IncludeInTotals: true},
