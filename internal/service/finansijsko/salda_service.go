...
type SaldaService interface {
	GetSaldaPojedinacnihKonta(c *gin.Context, tbl *domain.TableData, getTotalRecords bool, pageSize, page int) error
	GetSaldaGrupeKonta(c *gin.Context, tbl *domain.TableData, getTotalRecords bool, pageSize, page int) error
	GetSaldaPartneriList(c *gin.Context, tbl *domain.TableData, getTotalRecords bool, currentPage int, pageSize int) error
	GetSaldaTotalValues(c *gin.Context) (domain.SaldaDto, error)
	GetSaldaKlase5i6TotalValues(c *gin.Context) (domain.SaldaDto, error)
	ProcessSaldaPartneriDetails(c *gin.Context, idPartneri int64, tblKonta, tblDetalji *domain.TableData) error
	GetSaldaPartneriPrelomljeno(c *gin.Context, tbl *domain.TableData, getTotalRecords bool, currentPage int, pageSize int) error
	GetSaldaKlase5i6Analitika(c *gin.Context, tbl *domain.TableData, getTotalRecords bool, currentPage int, pageSize int) error
	SaldaKlase5i6MT(c *gin.Context, tbl *domain.TableData, getTotalRecords bool, currentPage int, pageSize int) error
	SaldaPoKomercijalistima(c *gin.Context, tbl *domain.TableData, getTotalRecords bool, currentPage int, pageSize int) error
	RealizacijaKomercijalisti(c *gin.Context, tbl *domain.TableData, getTotalRecords bool, currentPage int, pageSize int) error

	CheckSaldaGrupeParameters(c *gin.Context, requiredFields []string) (fieldsError []domain.FieldError)
	CheckSaldaParameters(c *gin.Context, requiredFields []string) []domain.FieldError
...
func (s *SaldaResource) GetSaldaPojedinacnihKonta(c *gin.Context, tbl *domain.TableData, getTotalRecords bool, pageSize, page int) error {
	// Get user session from context
	userSession := domain.GetSessionFromContext(c)
	if userSession == nil {
...
func (s *SaldaResource) GetSaldaPartneriList(c *gin.Context, tblPartneri *domain.TableData, getTotalRecords bool, currentPage int, pageSize int) error {
	// Get user session from context
	userSession := domain.GetSessionFromContext(c)
	if userSession == nil {
		return fmt.Errorf("user session not found")
...
func (s *SaldaResource) GetSaldaKlase5i6Analitika(c *gin.Context, tbl *domain.TableData, getTotalRecords bool, currentPage int, pageSize int) error {
	common.SetupTablePagination(tbl, currentPage, pageSize)
	// Get user session from context
	userSession := domain.GetSessionFromContext(c)
	if userSession == nil {
	...
	// Create first query (opening balance - tipdok = '00')
	qb := common.NewQueryBuilder(`SELECT f.konto, f.sifra, fkpl.naziv, coalesce(m.mtroska, '') as mtroska,
	...
	qb.AddCondition("f.konto", odkonta, ">=")
	qb.AddCondition("f.konto", dokonta, "<=")
...
func (s *SaldaResource) SaldaKlase5i6MT(c *gin.Context, tbl *domain.TableData, getTotalRecords bool, currentPage int, pageSize int) error {
	common.SetupTablePagination(tbl, currentPage, pageSize)
	// Get user session from context
	userSession := domain.GetSessionFromContext(c)
	if userSession == nil {
	...
	// Create first query (opening balance - tipdok = '00')
	qb := common.NewQueryBuilder(`SELECT f.konto, fkpl.naziv, coalesce(f.mestotrid, 0) as mestotrid,
    COALESCE(SUM(CASE WHEN f.kat = '1' OR f.kat = '2' THEN f.iznos ELSE 0 END), 0) as dug,
    COALESCE(SUM(CASE WHEN f.kat = '3' OR f.kat = '4' THEN f.iznos ELSE 0 END), 0) as pot
    FROM fpro f`)
	qb.AddJoin(" left join fkpl on fkpl.idfkpl = f.idfkpl")
	...
	qb.AddGroupBy("f.konto, fkpl.naziv, f.mestotrid")
	if !getTotalRecords {
		qb.SetLimit(pageSize)
		qb.SetOffset((currentPage - 1) * pageSize)
	}
...
func (s *SaldaResource) SaldaPoKomercijalistima(c *gin.Context, tbl *domain.TableData, getTotalRecords bool, currentPage int, pageSize int) error {
	// Get user session from context
	userSession := domain.GetSessionFromContext(c)
	if userSession == nil {
		return fmt.Errorf("user session not found")
...
func (s *SaldaResource) RealizacijaKomercijalisti(c *gin.Context, tbl *domain.TableData, getTotalRecords bool, currentPage int, pageSize int) error {
	// Get user session from context
	userSession := domain.GetSessionFromContext(c)
	if userSession == nil {
		return fmt.Errorf("user session not found")
...
func (s *SaldaResource) GetSaldaKlase5i6TotalValues(c *gin.Context) (domain.SaldaDto, error) {
	var totals domain.SaldaDto
